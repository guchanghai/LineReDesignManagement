...
BOOL CListCtrlEx::DisplayEditor(int nItem, int nSubItem)
{
	...
	CEdit* editor = dynamic_cast<CEdit*>(m_pEditor->m_pWnd);
	if( editor )
		editor->SetSel(0,-1);
...
LRESULT CListCtrlEx::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	if( WM_KEYDOWN == message )
	{
		if( (int)wParam == 68 )
		{
			int select = this->GetSelectionMark();
			if( select != -1 )
			{
...
void CListCtrlEx::ResetNumber()
{
	...
	{
		temp.Format(L"%d",i+1);
		this->SetItemText(i,0,temp.GetBuffer());
	}
}
